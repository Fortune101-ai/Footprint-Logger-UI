...
const addActivity = async (req, res, next) => {
    try {
        ...
        const factorData = emissionFactors[category] && emissionFactors[category][activity];
        if (!factorData) {
...
const getUserActivities = async (req, res, next) => {
    try {
        ...
        console.error(error);
        res.status(500).json({message: 'Internal server error getting user activities'}); 
    }
};
...
const deleteActivity = async (req,res) => {
    try {
       ...
       console.error(error);
       res.status(500).json({message: 'Internal server error deleting activity'});  
    }
}
